...
class LoadNewsIntent(
  private val source: String,
  private val newsRepository: NewsRepository): ObservableIntent<NewsModel>() {

  override fun invoke(): Observable<Reducer<NewsModel>> = newsRepository.news(source)
    .concatMap(::success)
  ...
  private fun success(resource: Resource<List<News>>): Observable<Reducer<NewsModel>> = when(resource) {
    is Resource.Success<List<News>> -> Observable.just(
      { o -> o.copy(state = Operation(REFRESH), data = resource.data ?: emptyList(), totalSize = resource.totalResult ?: 0) },
  ...
  private fun initial(): Reducer<NewsModel> = { o -> o.copy(state = Operation(REFRESH), data = emptyList(), totalSize = 0) }
}